...
const TIENDAS_NUEVA_COLECCION = ["Zara", "PullAndBear", "Bershka"];
...
async function cargarCategoriasCatalogo() {
    ...
    try {
        const categoriasSeleccionadasAntes = Array.from(
        ...
        const tiendasParaBuscar = filtros.tiendas.length > 0
            ? filtros.tiendas
            : TIENDAS_NUEVA_COLECCION;

        const categoriasSet = new Set();

        for (const tienda of tiendasParaBuscar) {
            const params = new URLSearchParams();

            params.append("tienda", tienda);
            params.append("nuevaColeccion", "true");
            ...
            const response = await fetch(`${BASE_URL}/productos/catalogo/categorias?${params.toString()}`, {
                method: "GET",
                credentials: "include"
            });
        ...
        const categoriasFinales = Array.from(categoriasSet).sort((a, b) => {
        ...
        renderizarCategoriasFiltro(categoriasFinales, categoriasSeleccionadasAntes);
...
function renderizarCategoriasFiltro(categorias, categoriasSeleccionadasAntes = []) {
    listaCategoriasFiltro.innerHTML = "";
    ...
    categorias.forEach(categoria => {
        ...
        input.checked = categoriasSeleccionadasAntes.includes(categoria);
...
function construirUrlCatalogo() {
    ...
    filtros.tiendas.forEach(tienda => {
        params.append("tienda", tienda);
    });
...
async function cargarProductosCatalogo(reiniciar = false) {
    ...
    try {
        ...
        if (reiniciar) {
            productosNuevaColeccion = [...productos];
        } else {
            productosNuevaColeccion = [...productosNuevaColeccion, ...productos];
        }
...
function mostrarErrorProductos() {
    ...
    if (grid && productosNuevaColeccion.length === 0) {
...
function renderizarProductos(productos, reiniciar = true) {
    ...
    if (productos.length === 0 && reiniciar) {
        grid.innerHTML = `<p class="sin-resultados">No hay productos de nueva colección con esos filtros.</p>`;
        return;
    }
...
function crearCardProducto(producto) {
    ...
    card.className = "tarjeta-nueva";

    const imgWrapper = crearCarruselImagenesProducto(producto);

    const badgeTienda = document.createElement("span");
    badgeTienda.className = "badge-tienda";
    badgeTienda.textContent = producto.tienda?.nombre || "Nueva colección";

    imgWrapper.appendChild(badgeTienda);
    ...
    infoProducto.append(linkProducto, categoria, productoFooter, miniMenuTalla);
...
function configurarFiltros() {
    document.querySelectorAll('input[data-tipo="tienda"], input[data-tipo="genero"]').forEach(input => {
...
async function iniciarNuevaColeccion() {
    try {
