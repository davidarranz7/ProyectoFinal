...
function renderizarProductos(productos, reiniciar = true) {
    ...
    contador.textContent = `WOMAN COLLECTION / ${productos.length} PRODUCTOS`;
...
async function iniciarMujeres() {
    try {
        ...
        productosMujer = mezclarArray([
            ...productosZara,
            ...productosPull,
            ...productosBershka
        ]).filter(p => p.seccion && p.seccion.toLowerCase().includes("mujer"));

        renderizarProductos(productosMujer, true);
