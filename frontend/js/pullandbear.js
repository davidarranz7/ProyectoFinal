...
function renderizarProductos(productos, reiniciar = true) {
    ...
    contador.textContent = `NEW DROPS / ${productos.length} PRODUCTOS`;
...
async function iniciarPullAndBear() {
    try {
        ...
        productosPull = mezclarArray([...productos]);
        renderizarProductos(productosPull, true);
