...
async function cargarProductosCatalogo(reiniciar = false) {
    ...
    try {
        ...
        if (reiniciar) {
            productosBershka = [...productos];
        } else {
            productosBershka = [...productosBershka, ...productos];
        }
...
function mostrarCargandoProductos() {
    ...
    if (contador) {
        contador.textContent = "TREND EDIT / CARGANDO...";
    }
...
function mostrarErrorProductos() {
    ...
    if (contador) {
        contador.textContent = "TREND EDIT / ERROR";
    }

    if (grid && productosBershka.length === 0) {
...
function renderizarProductos(productos, reiniciar = true) {
    ...
    contador.textContent = `TREND EDIT / ${totalProductosCatalogo} PRODUCTOS`;
...
function crearCardProducto(producto) {
    ...
    card.className = "tarjeta-bershka";
